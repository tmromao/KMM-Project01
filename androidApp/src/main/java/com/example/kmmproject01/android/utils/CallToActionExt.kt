...
import com.example.kmmproject01.utils.CallToAction
...
fun openEmail(context: Context, emailContext: CallToAction.Email, errorMessage: String) {
    ...
    intent.putExtra(Intent.EXTRA_EMAIL, arrayOf(emailContext.address.toString()/**emailContext.address.localized*/))
    intent.putExtra(Intent.EXTRA_SUBJECT, emailContext.subject.toString() /**emailContext.subject.localized*/)
...
fun callSupport(context: Context, callContext: CallToAction.Call, errorMessage: String) {
    val phoneUri = Uri.parse("tel:${callContext.number.toString() /**callContext.number.localized*/}")
